...
describe "Seeds.rb regression test" do
  ...
  context "incremental migrations and seeds.rb preservation" do
    ...
    it "preserves migration order when generating seeds.rb" do
      # Mock Rails environment for this test
      allow(Rails).to receive(:env).and_return(ActiveSupport::StringInquirer.new("development"))

      # Create models in a specific order that would be wrong if sorted alphabetically
      # If alphabetical: Product, User, UselessModel
      # But migration order should be: User, Product, UselessModel (registration order)

      # Step 1: Register and create User first (earliest migration)
      # User doesn't have created_at, so it will be ordered by registration order
      User.create!(username: "first_user")
      SeedMigration.register User

      # Step 2: Register and create Product second
      # Product has created_at, create it with older timestamp
      Product.create!(created_at: 2.hours.ago)
      SeedMigration.register Product

      # Step 3: Create UselessModel data but don't register (will be discovered)
      # This should come last based on created_at timestamp ordering
      UselessModel.create!(created_at: 1.hour.ago)

      # Capture warnings (UselessModel will trigger warning)
      logger_output = StringIO.new
      allow(SeedMigration::Migrator).to receive(:logger).and_return(Logger.new(logger_output))
      ...
      # Find the positions of each model in the file
      user_pos = contents.index("User.create")
      product_pos = contents.index("Product.create")
      useless_pos = contents.index("UselessModel.create")

      expect(user_pos).to be < product_pos, "User should come before Product (registration order preserved)"
      expect(product_pos).to be < useless_pos, "Product should come before UselessModel (registered before discovered)"

      # Verify that the warning mentions the unregistered model
      warning_text = logger_output.string
      expect(warning_text).to include("Found seed data for unregistered models: UselessModel")

      # This would fail if alphabetical: Product(0) < UselessModel(1) < User(2)
      # But should pass with proper order: User(0) < Product(1) < UselessModel(2)
    end
  end
end
